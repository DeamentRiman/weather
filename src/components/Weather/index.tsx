...
const Weather: React.FC = () => {
    ...
    return (
        <section className="appMainWeather">
            ...
            {weatherData.base === weatherTemplateObj.base ? (
                <p className="appMainWeatherNoData"></p>
            ) : (
                <section className="appMainWeatherData">
                    <div className="appMainWeatherPictire">
                        <img
                            className="appMainWeatherImg"
                            src={`http://openweathermap.org/img/wn/${weatherData.weather[0].icon}@2x.png`}
                            alt="Изображение погоды"
                        />
                    </div>
                    <div className="appMainWeatherWrapper">
                        <p className="appMainWeatherPlace">
                            Location: {weatherData.name},{' '}
                            {weatherData.sys.country}
                        </p>
                        <p className="appMainWeatherClouds">
                            Clouds: {weatherData.clouds.all} %
                        </p>
                        <p className="appMainWeatherVisibility">
                            Visibility: {weatherData.visibility} m
                        </p>
                        <p className="appMainWeatherHumidity">
                            Humidity: {weatherData.main.humidity} %
                        </p>
                    </div>
                    <div className="appMainWeatherWrapper">
                        <p className="appMainWeatherTemp">
                            Temperature:{' '}
                            {Math.round(weatherData.main.temp) - 273}
                            <sup>o</sup>
                        </p>
                        <p className="appMainWeatherTempRange">
                            Min / Max:{' '}
                        ...
                        <p className="appMainWeatherTempFeels">
                            Feels Like:{' '}
                            {Math.round(weatherData.main.feels_like) - 273}
                            <sup>o</sup>
                        </p>
                        <p className="appMainWeatherPressure">
                            Pressure:{' '}
                            {Math.round(
                                (weatherData.main.pressure * 100) / 133
                            )}{' '}
                            mmhg
                        </p>
                    </div>
                    <div className="appMainWeatherWrapper">
                        <p className="appMainWeatherWind">
                            Wind: {weatherData.wind.speed} m/s
                        </p>
                        <p className="appMainWeatherWindGust">
